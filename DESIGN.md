...
These patterns exist in the current codebase and must be deleted in the redesign:

| Pattern | File(s) | Action |
|---------|---------|--------|
| `isDark` state + theme toggle | `page.tsx` | Delete entirely |
| Theme toggle button (top-right) | `page.tsx:210-221` | Delete |
| `localStorage` theme persistence | `page.tsx` | Delete |
| All `dark:` Tailwind variants | all components | Convert `dark:xxx` to the actual warm-dark value; delete light-mode base |
| Radial gradient overlays | `layout.tsx:43-44` | Delete both gradient divs |
| `backdrop-blur-sm` / `backdrop-blur-md` | `PortfolioSummary.tsx`, `ArtistCard.tsx` | Remove |
| `rounded-[2.5rem]` / `rounded-3xl` | `ArtistCard.tsx`, `PortfolioSummary.tsx` | Replace with `rounded-none` |
| `bg-white/60` / `bg-white/80` | `ArtistCard.tsx` | Replace with `bg-[--surface]` |
| `text-cyan-*` / `text-emerald-*` | all | Replace with `--text`, `--muted`, `--accent` per role |
| `font-serif dark:font-mono` dual-font | all | Single font per role (see typography table) |
| `Inter` font | `layout.tsx` | Remove from next/font imports |
| `JetBrains_Mono` font | `layout.tsx` | Replace with `IBM_Plex_Mono` |
| `bg-[#fcfaf6]` light background | `layout.tsx:41` | Remove (dark-only) |
| `selection:bg-lime-100` | `layout.tsx:41` | Replace with `selection:bg-[--surface2]` |
| `PortfolioSummary` genre bars (progress bars) | `PortfolioSummary.tsx` | Replace with depth assessment block |

---

## Implementation Tasks (ordered)

1. **`layout.tsx`** — Update fonts (Playfair Display + IBM Plex Mono + IBM Plex Serif), remove Inter + JetBrains Mono, delete gradient overlays, fix body classes to dark-only.
2. **`globals.css`** — Add CSS custom properties (all `--` tokens above). Remove light-mode base styles.
3. **`page.tsx`** — Delete light mode system (isDark, toggle button, localStorage theme). Rebuild landing page (blank, wordmark, bare input). Rebuild results header (fixed top bar). Implement depth assessment prose block. Implement staged reveal animation.
4. **`PortfolioSummary.tsx`** — Replace with new `DepthAssessment` component (score number + prose + genre breakdown in IBM Plex Mono, no progress bars).
5. **`ArtistCard.tsx`** — Strip rounded corners, blur, white backgrounds. Apply warm-dark surface. Update typography to Playfair Display names + IBM Plex Mono data.
6. **`ArtistList.tsx`** — Apply card stagger animation. Update sort controls to warm-dark styling.
7. **`IcebergVisual.tsx`** — Verify it renders correctly in warm-dark (likely just color token updates).

---

## Not In Scope

- Light mode: will not be rebuilt.
- PWA / manifest: untouched for now (cosmetic only).
- Sharing URL system: unchanged (functionality stays, just restyled).
- Backend API: no changes needed for design (depth_score already returned; prose computed client-side).
