...
export default function ArtistCard({ artist, rank, stickinessThreshold }: ArtistCardProps) {
  ...
  return (
    <motion.div 
      ...
      className={`relative bg-white/60 dark:bg-cyan-950/20 border border-emerald-500/10 dark:border-cyan-500/20 rounded-[2.5rem] dark:rounded-none p-8 md:p-10 shadow-[0_4px_25px_rgba(0,0,0,0.02)] dark:shadow-[0_0_20px_rgba(6,182,212,0.05)] backdrop-blur-sm transition-all duration-1000 cursor-pointer overflow-hidden flex flex-col group`}
    >
        ...
        <div className="flex justify-between items-start leading-relaxed">
          <div className="flex flex-col gap-1 pr-6 flex-1">
            <div className="flex items-center gap-2">
              <h3 className="text-2xl font-serif dark:font-mono italic dark:not-italic text-neutral-900 dark:text-cyan-50 leading-[1.4] transition-all duration-1000">
                {artist.name}
              </h3>
              {isHighConviction && (
