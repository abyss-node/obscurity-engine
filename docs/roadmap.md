...
| **Underexplored-mult (loosen)** | ⚠️ **Partial win, re-engagement only.** Raising `underexplored_mult` 1→~3-4 (the threshold is already per-user: mean-plays × mult) lifts obscW +67% / precision +45%, obscurity preserved, MRR holds. But the *entire* gain is **re-engagement** (replaying obscure artists the user already half-knows); pure-discovery hits stay flat at 13. Shippable as a re-engagement amplifier if that's in scope — a per-user-relative default (~mult 2, balanced) or a user "discovery appetite" control. |
| **Adaptive backfill (auto-tune the mult)** | ❌ **A/B'd NO.** Tried to auto-set re-engagement per user (discovery-first reorder; and tighten reach-rich users). Reorder captures the gain but tanks MRR (0.125→0.086, buries gems). Pool-tighten preserves MRR but barely moves obscW (0.010) — because **you can't identify reach-rich users at inference**: nearly everyone has plentiful discovery *candidates*, just few that get *adopted* (unobservable). Automation doesn't beat the flat per-user-relative bound. Lever kept default-off (`novelty_model="adaptive"`). |
