/// Phase 3 (track mode): score, filter, and rank Spotify recommendation candidates.
///
/// Scoring formula:
///   conviction  = batch_appearances / total_batches   (0–1, cross-batch signal)
///   obscurity   = (100 - spotify_popularity) / 100    (0–1, inverted popularity)
///   stickiness  = lastfm_playcount / lastfm_listeners  (repeat-listen ratio)
///   composite   = conviction × obscurity × stickiness × 10_000
///
/// Last.fm track.getInfo is called for each candidate to:
///   - Filter tracks the user already listens to (userplaycount > 0)
///   - Get listener count for the depth score
///   - Get genre tags for taste_alignment and diversity enforcement
...
pub async fn score_and_rank(
    ...
    for (_key, (artist, track, batch_appearances, popularity)) in candidate_map {
        ...
        futures.push(tokio::spawn(async move {
            ...
            (artist, track, batch_appearances, popularity, info)
        }));
    }

    let mut scored: Vec<TrackDiscoveryItem> = Vec::new();
    while let Some(result) = futures.next().await {
        if let Ok((artist, track_name, batch_appearances, popularity, Ok(info_resp))) = result {
            if let Some(item) = score_candidate(
                info_resp.track, artist, track_name, batch_appearances, total_batches, popularity,
            ) {
                scored.push(item);
            }
        }
...
fn score_candidate(
    ...
    artist: String,
    track_name: String,
    batch_appearances: usize,
    total_batches: usize,
    popularity: u32,
) -> Option<TrackDiscoveryItem> {
    if info.user_plays() > 0 { return None; }
    if info.listeners > MAX_LISTENER_CEILING { return None; }

    let conviction = if total_batches > 0 {
        batch_appearances as f64 / total_batches as f64
    } else {
        0.0
    };
    let obscurity = (100u32.saturating_sub(popularity)) as f64 / 100.0;
    let stickiness = info.stickiness();
    let composite = conviction * obscurity * stickiness * 10_000.0;
